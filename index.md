...
[漢文句形トレーナー](https://kukeitrainer.netlify.app)
...
## 白文と書き下し文から訓点付きの文書を出力するWebアプリ。 
[訓点工房試行版](https://kukeitrainer.netlify.app/editor/)

漢文句形トレーナーのデータを作るために作成した付属アプリを発展させたものです。
訓点をつけた電子文書を作るのは大変ですが、白文と書き下し文を用意するだけで、黒板投影用の訓点付きのページが表示できます。
odf出力もついているので、一太郎でもWordでも読み込むことができます。

## 黒板にプロジェクタで投影するための横長（縦長）のPDFを作るアプリ　

[PDF2Board](https://pdf2board.netlify.app)

古文などを投影するために作ったアプリ、複数ページに分かれてしまった本文は、ページめくりをするとき、直前の本文が見えなくなってしまいますが、複数ページを結合して横長（縦長）に変換することで、本文全体をページめくりすることなく、少しずつ学習できます。白背景のページを、黒板に直接投影するために色変換をする機能も搭載しています。
